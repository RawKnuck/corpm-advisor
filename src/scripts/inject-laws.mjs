...
// Format the 48 Laws into a clean simulated essay block so RAG parses it seamlessly
const contentMarkdown = lawsOfPower.map(l => {
  return `### Law ${l.law}: ${l.title}\n\n${l.summary}`;
}).join('\n\n');

const lawsEssay = {
  title: "The 48 Laws of Power",
  url: "https://corpmachiavelli.com/the-48-laws-of-power/",
  content: contentMarkdown
};

// Check if already injected, if yes replace it, else push
const existingIdx = essays.findIndex(e => e.title === "The 48 Laws of Power");
if (existingIdx !== -1) {
  essays[existingIdx] = lawsEssay;
} else {
  essays.push(lawsEssay);
}

fs.writeFileSync(essaysPath, JSON.stringify(essays, null, 2));
console.log('Successfully injected the 48 Laws of Power into the knowledge base.');
